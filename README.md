...
- Archive your template in a `.tgz` file extension format to onboard to your private catalog. The size of the `.tgz` must be `<=40MB`. **Note** If you .tgz file size if greater than 40 MB, Use `rm -rf .git .gitignore` command to reduce the size of the `.tgz` file and create `tar czfv <reponame>.tgz .`.
